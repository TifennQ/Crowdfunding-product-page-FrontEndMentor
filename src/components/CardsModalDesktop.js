...
//TODO : changer le clique : permettre de fermer la modal en clique à l'extérieur ??
...
const CardsModalDesktop = ({cards, pledgeIsSelected, handleClick, handleCompletePledge}) => {
    ...
    const handleHover = (id, bool, left) => {
        if(left !== "0") {
                            ...
                            <div key={id} className={`${pledgeIsSelected === id ? "border-2 rounded-lg px-5 py-6 my-5 pledge_selected" : "border-2 rounded-lg px-5 py-6 my-5"}`}>
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center">
                                        <div id={`selectButton${id}`}  onMouseEnter={() => handleHover(id, true, left)} onMouseLeave={() => handleHover(id, false)} onClick={() => handleClick(id, left)} className={`${left === "0" ? "relative border-2 rounded-full w-8 h-8 mr-4 cursor-not-allowed" : " relative border-2 rounded-full w-8 h-8 mr-4 cursor-pointer selectButton"}`}>
                                            <div className={`${pledgeIsSelected === id ? "rounded-full w-5 h-5 cursor-pointer selected" : "rounded-full w-5 h-5 cursor-pointer"}`}></div>
                                        </div>
                                        <h4 id={`selectTitle${id}`} onMouseEnter={() => handleHover(id, true, left)} onMouseLeave={() => handleHover(id, false)}  onClick={() => handleClick(id, left)} className={`${left === "0" ? "font-bold gray_disabled mr-4" : "font-bold mr-4 cursor-pointer selectTitle"}`}>{title}</h4>
